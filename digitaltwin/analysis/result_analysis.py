...
OpenSim 结果文件与标准运动切片的对齐/统计工具（纯分析层）。
...
用途：
  ...
  - 从 pipeline results / 切片缓存中取出指定 load 的运动切片。
...
分层约定：
  - 本模块不再依赖 Subject / MultiLoadPipeline；
  - 编排逻辑（运行流水线、带缓存的切片装载、动作窗口）见
    digitaltwin/pipelines/standard_analysis.py，通过参数传入 pipeline results。
"""
import os
import numpy as np
import pandas as pd
...
# ============================================================
#  以下编排逻辑已移至 digitaltwin/pipelines/standard_analysis.py：
#    run_standard_data_pipeline / load_or_create_cutted_pipeline_results /
#    get_action_windows
#  本模块只保留纯分析函数，pipeline results 由调用方传入。
# ============================================================
