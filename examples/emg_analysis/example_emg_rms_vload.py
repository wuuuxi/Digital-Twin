...
在 example_emg_rms.py 的基础上，额外加载并分析 JSON 配置中
variable_load_file.data 指定的变负载实验数据，与固定负载结果一起对比。
...
# ==============================================================
#  图 1：RMS vs Time（固定负载 + 变负载）
# ==============================================================

def plot_rms_vs_time_combined(pipeline, vload_results, muscles):
    """
    运动切片风格 RMS-时间图。
    ...
    for musc in muscles:
        ...
        for key in all_keys:
            ...
            for df_key in ['aligned_data', 'cutted_data']:
                df = d.get(df_key)
                if df is not None and rms_col in df.columns:
                    vals = df[rms_col].dropna().values
                    if len(vals) > 0:
    ...
    fig, axes = plt.subplots(n_muscles, n_cols,
                             figsize=(3 * n_cols, 2 * n_muscles),
                             squeeze=False)
    fig.suptitle('RMS vs Time: Fixed Loads + Variable Loads',
    ...
    for mi, musc in enumerate(muscles):
        ...
        for ci, key in enumerate(all_keys):
            ...
            # 灰色底层
            if ad is not None and rms_col in ad.columns and 'time' in ad.columns:
                valid = ~np.isnan(ad[rms_col].values)
                ax.plot(ad['time'].values[valid], ad[rms_col].values[valid],
                        'k-', alpha=0.3, linewidth=0.8)

            # 彩色切片
            if cd is not None and rms_col in cd.columns and 'time' in cd.columns:
                if 'cycle_id' in cd.columns:
                    ...
                    for ci2 in range(n_cyc):
                        ...
                        if 'movement_type' in seg.columns:
                            ...
                            vu = ~np.isnan(up[rms_col].values)
                            vd = ~np.isnan(dn[rms_col].values)
                            if np.any(vu):
                                ax.plot(up['time'].values[vu], up[rms_col].values[vu],
                                        '-', color=clr, linewidth=1.5)
                            if np.any(vd):
                                ax.plot(dn['time'].values[vd], dn[rms_col].values[vd],
                                        '--', color=clr, linewidth=1.5, alpha=0.6)
                        else:
                            valid = ~np.isnan(seg[rms_col].values)
                            if np.any(valid):
                                ax.plot(seg['time'].values[valid], seg[rms_col].values[valid],
                                        '-', color=clr, linewidth=1.5)
                else:
                    valid = ~np.isnan(cd[rms_col].values)
                    ax.plot(cd['time'].values[valid], cd[rms_col].values[valid],
...
# ==============================================================
#  图 2：RMS vs Position（固定 + 变负载）
# ==============================================================

def plot_rms_vs_position_combined(pipeline, vload_results, muscles):
    """
    RMS-位置散点图，固定负载用实心点，变负载用空心三角。
    """
    results = pipeline.results
    fixed_keys = sorted(results.keys(), key=lambda x: float(x))
    vload_keys = list(vload_results.keys())
    ...
    fig, axes = plt.subplots(n_rows, n_cols, figsize=(4 * n_cols, 3 * n_rows),
                             squeeze=False)
    fig.suptitle('RMS vs Position: Fixed + Variable Loads', fontsize=14)
    ...
    for idx, musc in enumerate(muscles):
        ...
        for i, lw in enumerate(fixed_keys):
            ...
            if cd is None or rms_col not in cd.columns:
                continue
            pos = cd['pos_l'].values
            rms = cd[rms_col].values
            valid = ~np.isnan(rms)
            if np.any(valid):
                ax.scatter(pos[valid], rms[valid],
...
# ==============================================================
#  图 3：3行×N列 Position-Vel/EMG/RMS 网格（固定 + 变负载）
# ==============================================================

def plot_grid_combined(pipeline, vload_results, muscles=None):
    """
    3行(Vel/EMG/RMS) × (n_fixed + n_vload)列 的散点图。
    固定负载列在前，变负载列在后。
    """
    results = pipeline.results
    ...
    for muscle_name in muscles:
        ...
        rms_col = f'rms_{muscle_name}'

        # 全局 Y 范围
        gv = [float('inf'), float('-inf')]
        ge = [float('inf'), float('-inf')]
        gr = [float('inf'), float('-inf')]
        ...
        for key in all_keys:
            ...
            if rms_col in cd.columns:
                rv = cd[rms_col].dropna().values
                if len(rv) > 0:
                    gr = [min(gr[0], np.min(rv)), max(gr[1], np.max(rv))]
        ...
        fig, axes = plt.subplots(3, n_cols, figsize=(3 * n_cols, 6),
                                 squeeze=False)
        fig.suptitle(f'Pos-Vel / EMG / RMS ({muscle_name}) '
        ...
        for ci, key in enumerate(all_keys):
            ...
            if rms_col in cd.columns:
                rms = cd[rms_col].values
                valid = ~np.isnan(rms)
                ax3.scatter(pos[pmask & valid], rms[pmask & valid],
                            alpha=0.5, s=10, color=CON_COLOR, label='Con')
                ax3.scatter(pos[nmask & valid], rms[nmask & valid],
                            alpha=0.5, s=10, color=ECC_COLOR, label='Ecc')
            ax3.set_ylim(gr[0], gr[1])
...
# ==============================================================
#  图 4：RMS 均值柱状图（固定 + 变负载）
# ==============================================================

def plot_rms_bar_combined(pipeline, vload_results, muscles):
    """
    ...
    fig, axes = plt.subplots(n_rows, n_cols, figsize=(5 * n_cols, 3 * n_rows),
                             squeeze=False)
    fig.suptitle('Mean RMS: Fixed Loads vs Variable Loads', fontsize=14)
...
def main():
    ...
    # --- 图 1: RMS vs Time ---
    plot_rms_vs_time_combined(pipeline, vload_results, muscles)

    # --- 图 2: RMS vs Position ---
    plot_rms_vs_position_combined(pipeline, vload_results, muscles)

    # --- 图 3: Position-Vel/EMG/RMS 网格 ---
    plot_grid_combined(pipeline, vload_results, ['VL', 'RF'])

    # --- 图 4: RMS 均值柱状图 ---
    plot_rms_bar_combined(pipeline, vload_results, muscles)
